...
function exportToExcel() {
    ...
    // Generate filename
    const studentName = studentNameInput.value ? studentNameInput.value.replace(/\s+/g, '_') : 'all_students';
...
function renderMealDistributionChart() {
    ...
    // If student name is provided, filter by that student
    if (studentNameInput.value) {
        records = dietRecords.filter(record => 
            record.studentName === studentNameInput.value
        );
    } else {
        // Otherwise use search filtered records or all records
        records = searchQuery 
            ? getFilteredRecords() 
            : dietRecords;
    }
